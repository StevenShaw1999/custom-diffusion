...
def create_custom_diffusion_lora_decouple(unet, freeze_model):
    ...
    def new_forward(self, hidden_states, **kwargs):
        ...
        query = self.to_q(hidden_states)
        context = context if context is not None else hidden_states
        key = self.to_k(context) + scale * self.to_k_lora(context)
        value = self.to_v(context) + scale * self.to_k_lora(context)
        ...
        if crossattn:
            ...
            value = value * mask_val + (1 - mask_val) * sub_value.mean(dim=1, keepdim=True)"""
                #value[num, item, :] = torch.mean(mask_new * value_dec, dim=1, keepdim=True)
                #value[num, item, :] = torch.mean(mask_val * sub_value, dim=1, keepdim=True)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="load delta ckpt of textural pretraining.",
    )
    parser.add_argument(
        "--initializer_token", type=str, default='ktn+pll+ucd+Øº+Øµ+ðŁĻ+ðŁĴĺ+ðŁĳĬ', help="A token to use as initializer word."
    )
...
class CustomDiffusionDataset(Dataset):
    ...
    def __init__(
        ...
        self.image_mask_pth = image_mask_pth
        ...
        self.flip_mask = transforms.RandomHorizontalFlip(hflip)
        

        self.image_transforms_instance = transforms.Compose(
            [
                self.flip,
        ...
        self.image_transforms_class = transforms.Compose(
            [
                self.flip,
                transforms.Resize(size, interpolation=transforms.InterpolationMode.BILINEAR),
                transforms.CenterCrop(size) if center_crop else transforms.RandomCrop(size),
    ...
    def __getitem__(self, index):
        ...
        if self.image_mask_pth is not None:
            instance_mask = Image.open(self.image_mask_pth)
        ...
        if random_scale < 0.6*self.size:
            ...
            if self.image_mask_pth is not None:
                instance_mask1 = instance_mask.resize((random_scale, random_scale), resample=self.interpolation)
                instance_mask1 = np.array(instance_mask1).astype(np.uint8)
                instance_mask1 = (instance_mask1 / 255).astype(np.float32)

                instance_mask =  np.zeros((self.size, self.size,3), dtype=np.float32)
                instance_mask[cx - random_scale // 2: cx + random_scale // 2, cy - random_scale // 2: cy + random_scale // 2, :] = instance_mask1
            ...
            if self.image_mask_pth is not None:
                instance_mask = instance_mask.resize((random_scale, random_scale), resample=self.interpolation)
                instance_mask = np.array(instance_mask).astype(np.uint8)
                instance_mask = (instance_mask / 255).astype(np.float32)
                instance_mask = instance_mask[cx - self.size // 2: cx + self.size // 2, cy - self.size // 2: cy + self.size // 2, :]
            ...
            if self.image_mask_pth is not None:
                if self.size is not None:
                    instance_mask = instance_mask.resize((self.size, self.size), resample=self.interpolation)
                instance_mask = np.array(instance_mask).astype(np.uint8)
                instance_mask = (instance_mask / 255).astype(np.float32)
            mask = np.ones((self.size // 8, self.size // 8))
        ########################################################################
        ...
        if self.image_mask_pth is not None:
            #instance_mask = instance_mask.resize((64, 64), resample=self.interpolation)
            example["instance_masks"] = torch.from_numpy(instance_mask).permute(2,0,1)
        ...
        if self.with_prior_preservation:
            ...
            example["class_images"] = self.image_transforms_class(class_image)
...
def main(args):
    ...
    controlnet = ControlNetModel.from_pretrained("/data1/jiayu_xiao/.cache/huggingface/hub/models--lllyasviel--sd-controlnet-hed/snapshots/6e9d172cfa3736c5a1519ed594b09237e1364726", revision=args.revision)
    ...
    if args.delta_ckpt is not None:
        diffuser_training.load_model_new(text_encoder, tokenizer, args.delta_ckpt)
       
    unet, lora_layers = create_custom_diffusion_lora_decouple(unet, args.freeze_model)
    ...
    train_dataset = CustomDiffusionDataset(
        ...
        hflip=args.hflip,
        image_mask_pth='/data1/jiayu_xiao/project/U-2-Net/test_data/u2net_results_custom/yann-lecun.png'
    )
    ...
    def collate_fn(examples):
        ...
        instance_mask = [example["instance_masks"] for example in examples]
        ...
        instance_mask = instance_mask.to(memory_format=torch.contiguous_format).float()
    ...
    if args.train_text_encoder or args.modifier_token is not None:
        ...
        lora_layers, unet, text_encoder, controlnet, apply_hed = accelerator.prepare(
            lora_layers, unet, text_encoder, controlnet, apply_hed
        )
    ...
    reverse_flag = False
    add = torch.LongTensor([49407] * 77).unsqueeze(0).repeat(2, 1).to(accelerator.device)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                instance_mask_ori = batch["instance_mask"].to(dtype=weight_dtype)

                
                

                #mask_pixels = batch["pixel_values"].to(dtype=weight_dtype) * (instance_mask_ori >= 0.05) +\
                #                    torch.ones_like(batch["pixel_values"], device=instance_mask_ori.device).to(dtype=weight_dtype) * (instance_mask_ori < 0.05)

                latents = vae.encode(batch["pixel_values"].to(dtype=weight_dtype)).latent_dist.sample()
                latents = latents * 0.18215
                
                instance_mask = Upsample(instance_mask_ori).mean(dim=1, keepdim=True)

                """image = instance_mask.cpu()
                ...
                """for numm in range(timesteps.shape[0]):
                    sudo_timesteps = -1
                    while sudo_timesteps < 0 or sudo_timesteps > 1:
                        sudo_timesteps = torch.normal(0, 3, (1,)) / 15 + ( 0.25 + global_step / args.max_train_steps * 0.5)
                    

                    sudo_timesteps = (sudo_timesteps.clamp_(0.001, 1) * 1000).long()
                    #torch.randn(,)
                    timesteps[numm] = 1000 - sudo_timesteps"""
                #print(timesteps)
                ...
                wher_instance = torch.where(new_batch_prompts == 49408)[1]
                #print(new_batch_prompts)
                #print(wher_instance)
                #exit()
                

                new_batch_prompts = torch.cat([new_batch_prompts, add], dim=0)

                """new_add = torch.range(1, 5).long().to(latents.device)
                new_batch_prompts[:, -5:] += new_add"""
                ...
                #rnd_zeros = torch.zeros(latents.shape[0], 1, 1, 1).to(latents.device)
                #rnd = (rnd - global_step / args.max_train_steps * 0.9) * 0.6
                #rnd = (rnd - global_step / args.max_train_steps * 0.8) * 0.6 good

                rnd = (rnd - 1 + (1 + math.cos(global_step * math.pi / args.max_train_steps)) / 2)

                #rnd = (rnd - global_step / args.max_train_steps) * 0.7 
                #rnd = (rnd - global_step / args.max_train_steps * 0.7) * 1.0 

                #print(rnd)
                rnd[1] = 0
                rnd = rnd.clamp_(0, 1)

                #rnd[0] = (1.0 - timesteps[0] / 1000)
                #rnd = 1
                #if rnd <= 0.5:
                ...
                """hh = (instance_mask_ori >= 0.05).squeeze().permute(1, 2, 0).cpu().numpy() * 255
                cv2.imwrite('mask.png', hh)
                hh = hed[0].squeeze().permute(1, 2, 0).cpu().numpy() * 255
                cv2.imwrite('mask_hed.png', hh)
                hh = (hed * instance_mask_ori)[0].squeeze().permute(1, 2, 0).cpu().numpy() * 255
                cv2.imwrite('masked_hed.png', hh)
                exit()"""
                #with torch.no_grad():
                if reverse_flag:
                    """rnd[0] = 0.0
                    ...
                    #                    down_block_additional_residuals = control_addition_down_inp).sample
                    ...
                    model_pred = unet(noisy_latents, timesteps, encoder_hidden_states, cross_attention_kwargs={'wher2swap': wher_instance, 'decouple_wher': 3}).sample

                # no_grad 會好一些，用<new1> man作為controlnet輸入過強，用man有些弱，換成插值, 这个方案舍弃了
                ...
                if args.with_prior_preservation:
                    ...
                    """for b in range(len(model_pred)):
                        ...
                        # 随机去掉一个数
                        removed_num = random.choice(arr).to(latents.device)
                        arr = arr[arr != removed_num]

                        # 随机打乱顺序
                        #print(arr)
                        #print(model_pred[b, arr, :, :].reshape(3, -1).permute(1,0).size())
                    ...
                    loss = ((loss*instance_mask).sum([1, 2, 3])/instance_mask.sum([1, 2, 3])).mean()
                #exit()
                ...
                #print(text_encoder.module.get_input_embeddings().weight[49408:, :5])
